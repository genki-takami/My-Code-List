...
final class EditMainViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, DataReturn3 {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        case "settingLinkSegue":
            let urlViewController:EditLinkViewController = segue.destination as! EditLinkViewController
        ...
        case "listSegue":
            let contentsTableViewController:EditContentListViewController = segue.destination as! EditContentListViewController
        ...
        case "createNoticeSegue":
            let noticeTableViewController:EditNoticeListViewController = segue.destination as! EditNoticeListViewController
        ...
        case "createEventSegue":
            let eventsTableViewController:EditEventListViewController = segue.destination as! EditEventListViewController
            eventsTableViewController.mailAdress = self.mailAdress
            eventsTableViewController.password = self.password
        case "createMapSegue":
            let mapEditViewController:EditMapViewController = segue.destination as! EditMapViewController
        ...
        case "voteOptionSegue":
            let voteOptionViewController:VoteOptionPurchaseViewController = segue.destination as! VoteOptionPurchaseViewController
    ...
    func pushFirebase(){
        ...
        let db1 = firestore.collection(PathName.DraftPath).document(self.saveData.uuid)
        let db2 = firestore.collection(PathName.ListContentsID).document(self.saveData.uuid)
        let db3 = firestore.collection(PathName.ListEventsID).document(self.saveData.uuid)
        let db4 = firestore.collection(PathName.ListNoticeID).document(self.saveData.uuid)
        let db5 = firestore.collection(PathName.ListMapID).document(self.saveData.uuid)
        ...
        if !contents.isEmpty{
            for content in contents{
                ...
                ci.append("\(self.saveData.uuid)/\(PathName.ContentImagePath)/\(content.name).jpg")
            }
            if self.isNewObject {
        ...
        if !events.isEmpty{
            for event in events{
                ...
                for n in event.imageCaptions{
                    ...
                    ei.append("\(self.saveData.uuid)/\(PathName.EventImagePath)/\(event.eventTitle)/\(n.prefix(10)).jpg")
                }
        ...
        let mainData: [String: Any] = [
            ...
            "database" : [
                ...
                "notice": nc,
                "eventImage": ei,
                "icon" : self.saveData.uuid + "/" + PathName.FestivalIconImagePath + ".jpg",
                "background" : self.saveData.uuid + "/" + PathName.FestivalBackgroundImagePath + ".jpg"
            ],
        ...
        batch.commit() { error in
            if let _ = error {
                ...
                // アイコン画像
                imgdb.child(PathName.FestivalIconImagePath + ".jpg").putData(self.saveData.iconImage, metadata: storageMetadata) { (metadata, error) in
                    if let _ = error {
                        ...
                        // 背景画像
                        imgdb.child(PathName.FestivalBackgroundImagePath + ".jpg").putData(self.saveData.backgroundImage, metadata: storageMetadata) { (metadata, error) in
                            if let _ = error {
                                ...
                                if !contentsData.isEmpty{
                                    ...
                                    for i in contents{
                                        imgdb.child(PathName.ContentImagePath).child(i.name + ".jpg").putData(i.image, metadata: storageMetadata) { (metadata, error) in
                                            if let _ = error {
                                    ...
                                    for j in events{
                                        ...
                                        if !j.images.isEmpty{
                                            for (index,k) in j.images.enumerated(){
                                                imgdb.child(PathName.EventImagePath).child(j.eventTitle).child(captionList[index].prefix(10) + ".jpg").putData(k, metadata: storageMetadata) { (metadata, error) in
                                                    if let _ = error {
