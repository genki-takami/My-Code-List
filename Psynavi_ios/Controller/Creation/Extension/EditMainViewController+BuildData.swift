...
extension EditMainViewController: BuildData {
    
    /// データを構築する
    func buildDocumentData(_ contents: Results<ShopDisplay>, _ notices: Results<Notices>, _ events: Results<Event>, _ mapData: Map) -> [Any] {
        
        /// 各データを構築
        let content = buildContent(with: contents)
        let event = buildEvent(with: events)
        let notice = buildNotice(with: notices)
        let map = buildMap(with: mapData)
    ...
    /// コンテンツデータ
    private func buildContent(with contents: Results<ShopDisplay>) -> [Any] {
    ...
    /// イベントデータ
    private func buildEvent(with events: Results<Event>) -> [Any] {
    ...
    /// お知らせデータ
    private func buildNotice(with notices: Results<Notices>) -> [Any] {
    ...
    /// マップデータ
    private func buildMap(with mapData: Map) -> [Any] {
