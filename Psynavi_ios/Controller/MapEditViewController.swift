...
class MapEditViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate, UIViewControllerTransitioningDelegate, DataReturn2 {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let latitude = self.downloadLat, let longitude = self.downloadLon, let count = self.downloadableMarkers {
            ...
            if count > 0 {
                SVProgressHUD.show()
                Firestore.firestore().collection(Const.ListMapID).document(self.uuid).getDocument { (document, error) in
                    if let _ = error{
    ...
    @IBAction func longPressMap(_ sender: UILongPressGestureRecognizer) {
        ...
        // 詳細設定に行く
        let pinEditViewController = self.storyboard?.instantiateViewController(withIdentifier: "PinEditView") as! PinEditViewController
