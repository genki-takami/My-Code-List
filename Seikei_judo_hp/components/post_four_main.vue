<template>
  <div id="post_four_main">
    <h1 style="margin:0;text-align: center;">四大本戦の記録追加</h1>
    <label>年度</label><br>
    ...
    update() {
      ...
      axios.get('/four-main')
            .then(response => {
              ...
              // 更新する
              axios.patch('/four-main/data',{fields: this.matchList},{headers: {Authorization: `Bearer ${this.idToken}`}}
              ).then(response => {
                if (response) {
                  // データの更新成功
                  axios.patch('/four-main/idList?updateMask.fieldPaths=list',{fields: this.idArray},{headers: {Authorization: `Bearer ${this.idToken}`}}
                  ).then(response => {
                    if (response) {
                      // 試合IDの更新成功
                      const date = new Date();
