<template>
  <div id="post_tokyo_student_personal">
    <h1 style="margin:0;text-align: center;">東京学生柔道体重別選手権大会の記録追加</h1>
    <label>年度</label><br>
...
export default {
  ...
  methods: {
    ...
    update() {
      ...
      axios.get('/tokyo-student-personal')
            .then(response => {
              // 情報ゲット
              ...
              // 更新する
              axios.patch('/tokyo-student-personal/data',{fields: this.matchList},{headers: {Authorization: `Bearer ${this.idToken}`}}
              ).then(response => {
                if (response) {
                  // データの更新成功
                  axios.patch('/tokyo-student-personal/idList?updateMask.fieldPaths=list',{fields: this.idArray},{headers: {Authorization: `Bearer ${this.idToken}`}}
                  ).then(response => {
                    if (response) {
                      // 試合IDの更新成功
                      const date = new Date();
