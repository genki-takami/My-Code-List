<template>
  <div id="post_suginami">
    <h1 style="margin:0;text-align: center;">杉並区民大会の記録追加</h1>
    <label>年度</label><br>
    ...
    <div id="writing_area">
      ...
      <label>参加状況</label><br>
      <input type="text" v-model="status" placeholder="初参加"><br>
      <label>ラウンド</label><br>
...
export default {
  ...
  methods: {
    register(){
      this.statuses.push({stringValue:this.statuses});
    ...
    update() {
      ...
      axios.get('/suginami')
            .then(response => {
              ...
              // 更新する
              axios.patch('/suginami/data',{fields: this.matchList},{headers: {Authorization: `Bearer ${this.idToken}`}}
              ).then(response => {
                if (response) {
                  // データの更新成功
                  axios.patch('/suginami/idList?updateMask.fieldPaths=list',{fields: this.idArray},{headers: {Authorization: `Bearer ${this.idToken}`}}
                  ).then(response => {
                    if (response) {
                      // 試合IDの更新成功
                      const date = new Date();
