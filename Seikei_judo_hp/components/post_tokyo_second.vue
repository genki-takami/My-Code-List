<template>
  <div id="post_tokyo_second">
    <h1 style="margin:0;text-align: center;">二部大会の記録追加</h1>
    <label>年度</label><br>
    ...
    update() {
      ...
      axios.get('/tokyo-second')
            .then(response => {
              ...
              // 更新する
              axios.patch('/tokyo-second/data',{fields: this.matchList},{headers: {Authorization: `Bearer ${this.idToken}`}}
              ).then(response => {
                if (response) {
                  // データの更新成功
                  axios.patch('/tokyo-second/idList?updateMask.fieldPaths=list',{fields: this.idArray},{headers: {Authorization: `Bearer ${this.idToken}`}}
                  ).then(response => {
                    if (response) {
                      // 試合IDの更新成功
                      const date = new Date();
